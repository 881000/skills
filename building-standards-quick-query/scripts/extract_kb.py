...
# -*- coding: utf-8 -*-
"""
extract_kb.py —— 从 atlas/ 下的文字版图集 PDF 重建 references/*.txt 全文索引。

用途：一次性维护用。txt 是关键词快查索引（毫秒级），由 PDF 文字层导出，
每页以 "--- PDF页N ---" 分隔；渲染原图仍走 atlas/ 的源 PDF。
...
def main():
    ...
    assets_dir = args.assets_dir or os.path.abspath(os.path.join(here, "..", "atlas"))
